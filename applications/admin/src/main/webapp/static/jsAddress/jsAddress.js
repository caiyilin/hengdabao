var addressInit = function(_cmbProvince, _cmbCity, defaultProvince, defaultCity)
{
    ...
    function cmbSelect(cmb, str)
    {
        if(cmb.options.length>0){
            ...
            $(cmb).val(cmb.options[0].value).trigger("change");
        }
        for(var i=0; i<cmb.options.length; i++)
        {
            if(cmb.options[i].value == str)
            {
                cmb.options[i].selected = 'selected';
                $(cmb).val(cmb.options[i].value).trigger("change");
                return;
            }
        }
    }
...
var provinceList = [
    ...
    {name:'江苏', cityList:[
        ...
        {name:'苏州市', areaList:['市辖区','沧浪区','平江区','金阊区','虎丘区','吴中区','相城区','姑苏区','工业园区','高新区','吴江区','常熟市','张家港市','昆山市','太仓市']},
